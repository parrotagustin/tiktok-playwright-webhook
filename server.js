...
app.post("/debug-dom", async (req, res) => {
  ...
  if (!existsSync(storagePath)) {
    return res.json({ ok: false, error: "storageState no encontrado" });
  }
  ...
  try {
    ...
    // 2) Click botón comentarios (FIX DEFINITIVO)
    try {
    ...
    if (search_text) {
      const raw = bodyHtml.indexOf(search_text);
      const lower = bodyHtml.toLowerCase().indexOf(search_text.toLowerCase());
      debug.text_search = {
        search_text,
        found_raw: raw !== -1,
        index_raw: raw,
        found_lowercase: lower !== -1,
        index_lowercase: lower,
      };
    }
    ...
    return res.json({
      ...
      debug,
      dom: {
        bodySnippet: bodyHtml.slice(0, 8000),
      },
    });
  } catch (err) {
